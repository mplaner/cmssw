//
// Class: EGSuperClusterRegTreeMaker.cc
//
...
#include "Regression/RegressionTrees/plugins/EGSuperClusterRegTreeMaker.h"
...
EGSuperClusterRegTreeMaker::EGSuperClusterRegTreeMaker(const PSet& p):
    _geomInitialized(false)
{
    ...
    //FIXME _tree->Branch("nVtx"                    , &_nVtx                   , "nVtx/I");
    //FIXME _tree->Branch("rho"                     , &_rho                    , "rho/F");
    ...
    _scInputEB                 = consumes<reco::SuperClusterCollection>(p.getParameter<edm::InputTag>("superClusterSrcEB"));
    _scInputEE                 = consumes<reco::SuperClusterCollection>(p.getParameter<edm::InputTag>("superClusterSrcEE")); 
    ...
    //FIXME    _PrimaryVertex             = p.getParameter<edm::InputTag>("VtxLabel"); 
    _geninput                  = consumes<reco::GenParticleCollection>(p.getParameter<edm::InputTag>("genSrc"));    
    ...
    _matchingType              = p.getParameter<int>("matchingType");// 0=DR; 1=DE/E
}


void EGSuperClusterRegTreeMaker::analyze(const edm::Event& e, const edm::EventSetup& es) 
{
    if (!_geomInitialized) 
    {
        edm::ESHandle<CaloGeometry> theCaloGeometry;
        es.get<CaloGeometryRecord>().get(theCaloGeometry); 
        _caloGeometry = & (*theCaloGeometry);
        _geomInitialized = true;
    }
    ...
    //FIXME edm::Handle<reco::VertexCollection> PV;
    e.getByToken(_geninput,gens);
    e.getByToken(_scInputEB, ebSCs);  
    e.getByToken(_scInputEE, eeSCs);  
    //FIXME e.getByLabel(_PrimaryVertex, PV);
    ...
    */
    ...
    /*
      ...
      {
	if(!gens.isValid())
	  std::cout << "gen collection invalid" << std::endl;
	if(!ebSCs.isValid())
	  std::cout << "EB collection invalid" << std::endl;
	if(!eeSCs.isValid())
	  std::cout << "EE collection invalid" << std::endl;
	
	throw cms::Exception("PFSuperClusterRegTreeMaker")
	  << "Input collection was invalid!"
	  << std::endl;
      }
    */
...
void EGSuperClusterRegTreeMaker::processSuperClusterFillTree(const edm::Event& e, const edm::EventSetup& es, const reco::SuperCluster& sc, const reco::GenParticle& gen) 
{
    ...
    // FIXME: workaround (etaWidth and phiWidth are not stored in the SC)
    edm::Handle<EcalRecHitCollection> rhs;
    e.getByToken( (_scIsEB ? _ebReducedRecHitToken: _eeReducedRecHitToken) , rhs);
    const CaloSubdetectorGeometry* geom = (_scIsEB ? _caloGeometry->getSubdetectorGeometry(DetId::Ecal,EcalBarrel) : _caloGeometry->getSubdetectorGeometry(DetId::Ecal,EcalEndcap));
    SuperClusterShapeAlgo  SCShape( rhs.product(), geom);
    SCShape.Calculate_Covariances(sc);
    _scPhiWidth         = SCShape.phiWidth();
    _scEtaWidth         = SCShape.etaWidth();
    //_scPhiWidth         = sc.phiWidth();
    //_scEtaWidth         = sc.etaWidth();


    // sc seed information
    edm::Ptr<reco::CaloCluster> theseed = edm::Ptr<reco::CaloCluster>(sc.seed());
    _scSeedRawEnergy        = theseed->energy();
    _scSeedCalibratedEnergy = theseed->energy();//_calib->energyEm(*theseed,0.0,0.0,false);
    ...
    {
        pclus = edm::Ptr<reco::CaloCluster>(*clus);
        ...
        //_clusterCalibEnergy.get()[iclus]    = _calib->energyEm(*pclus,0.0,0.0,false);
        ...
        //        pclus->eta(),
        //        pclus->phi());
        //_clusterInDynDPhi.get()[iclus] = (int) MK::inDynamicDPhiWindow(PFLayer::ECAL_BARREL == pclus->layer(),
        //            theseed->phi(),
        //            pclus->energy(),
        //            pclus->eta(),
        //            pclus->phi());
...
void EGSuperClusterRegTreeMaker::processNoMatchFillTree(const edm::Event& e, const edm::EventSetup& es, const reco::GenParticle& gen) 
{
...
void EGSuperClusterRegTreeMaker::setTreeArraysForSize(const size_t N_ECAL, const size_t N_PS) 
{
...
bool EGSuperClusterRegTreeMaker::IsLinkedByRecHit(const edm::Ptr<reco::CaloCluster>& the_seed, const edm::Ptr<reco::CaloCluster>& x, const double threshold, const double majority)
{
...
void EGSuperClusterRegTreeMaker::matchSuperClustersToGen(const reco::SuperClusterCollection& ebSCs, const reco::SuperClusterCollection& eeSCs, const reco::GenParticleCollection& gens)
{
...
DEFINE_FWK_MODULE(EGSuperClusterRegTreeMaker);
