//
// Class: GedGsfElectronRegTreeMaker.cc
//
...
#include "Regression/RegressionTrees/plugins/GedGsfElectronRegTreeMaker.h"
...
GedGsfElectronRegTreeMaker::GedGsfElectronRegTreeMaker(const PSet& p) 
{
    _calib.reset(new PFEnergyCalibration());
    _N_ECALClusters = 1;
    _N_PSClusters   = 1;
    _tree = _fs->make<TTree>("RegressionTree","Regression tree for GedGsfElectrons");
    ...
    // track-combination information
    _tree->Branch("trkMomentum"           , &_trkMomentum,         "trkMomentum/F");
    _tree->Branch("trkMomentumError"      , &_trkMomentumError,    "trkMomentumError/F");
    _tree->Branch("trkMomentumRelError"   , &_trkMomentumRelError, "trkMomentumRelError/F");
    _tree->Branch("eleClass"              , &_eleClass,            "eleClass/I");
    _tree->Branch("eleEcalDriven"         , &_eleEcalDriven,       "eleEcalDriven/I");
    _tree->Branch("eleTrackerDriven"      , &_eleTrackerDriven,    "eleTrackerDriven/I");
    ...
    _eleInput                  = p.getParameter<edm::InputTag>("gsfElectronSrc");
...
void GedGsfElectronRegTreeMaker::analyze(const edm::Event& e, const edm::EventSetup& es) 
{
    ...
    e.getByLabel(_eleInput, eles);  
    e.getByLabel(_PrimaryVertex, PV);
    _eventNumber = e.id().event();
    _nVtx = PV->size();
    _scIndex = 0;
    _matchedParticles.clear();
    _matchedElectrons.clear();

    // Get number of electrons with pT>20GeV
    _nElectrons = 0;
    if(eles.isValid())
    {
        for( const auto& ele : *eles) 
        {
            if(ele.pt()>20.) _nElectrons++;
        }
    }
    ...
    if(eles.isValid() && gens.isValid())
    {
        matchElectronToGen(*eles, *gens);
    }
    else 
    {
        throw cms::Exception("GedGsfElectronRegTreeMaker")
    ...
    for(;it!=itE;++it)
    {
        ...
        if(it->second!=-1)
        {
            if(_matchedElectrons[it->second].superCluster().isNull())
            {
                throw cms::Exception("GedGsfElectronRegTreeMaker")
                    << "Invalid electron supercluster"
                    << std::endl;
            }
            processSuperClusterFillTree(e, es, _matchedElectrons[it->second], *(_matchedElectrons[it->second].superCluster()), it->first);
        }
        else
        {
...
void GedGsfElectronRegTreeMaker::processSuperClusterFillTree(const edm::Event& e, const edm::EventSetup& es, const reco::GsfElectron& ele, const reco::SuperCluster& sc, const reco::GenParticle& gen) 
{
    ...
    _trkMomentum = ele.trackMomentumAtVtx().R();   
    _trkMomentumError = ele.trackMomentumError();
    _trkMomentumRelError = _trkMomentumError / _trkMomentum;
    _eleClass = -1;
    ...
    _eleEcalDriven    = ele.ecalDriven();
    _eleTrackerDriven =  ele.trackerDrivenSeed();


    _tree->Fill();
}

void GedGsfElectronRegTreeMaker::processNoMatchFillTree(const edm::Event& e, const edm::EventSetup& es, const reco::GenParticle& gen) 
{
...
void GedGsfElectronRegTreeMaker::setTreeArraysForSize(const size_t N_ECAL, const size_t N_PS) 
{
...
bool GedGsfElectronRegTreeMaker::IsLinkedByRecHit(const edm::Ptr<reco::PFCluster>& the_seed, const edm::Ptr<reco::PFCluster>& x, const double threshold, const double majority)
{
...
void GedGsfElectronRegTreeMaker::matchElectronToGen(const reco::GsfElectronCollection& eles, const reco::GenParticleCollection& gens)
{
    for( const auto& gen : gens ) 
    {
        if( abs(gen.pdgId()) == 11 &&  gen.status()==1) // looking at final state electrons 
        {
            double minDr = 1e6;
            double minDe = 1e6;
            double minDeDr = 1e6;
            ...
            double this_de;
            double this_dedr;
            const reco::GsfElectron* bestmatch = NULL;
            // loop on EB SCs
            for( const auto& ele : eles) 
            {
                math::XYZVector direction = ele.superCluster()->position() - gen.vertex();
                double energy = ele.superCluster()->energy();
                math::XYZVector momentum = direction.unit() * energy;
                math::XYZTLorentzVector physicalSCP4(momentum.x(), momentum.y(), momentum.z(), energy );
                //this_dr = reco::deltaR(gen,ele);
                this_dr = reco::deltaR(gen,physicalSCP4);
                //this_de = fabs(gen.energy()-ele.energy())/gen.energy();
                this_de = fabs(gen.energy()-energy)/gen.energy();
                ...
                //if(this_dr < _matchMaxDR && ele.energy()/gen.energy()>_matchMinERatio)
                if(this_dr < _matchMaxDR && energy/gen.energy()>_matchMinERatio)
                {
            ...
            if(bestmatch)
            {
                _matchedElectrons.push_back(*bestmatch);
                _matchedParticles.push_back( std::make_pair(gen, (int)_matchedElectrons.size()-1) );
            }
            else
            {
...
DEFINE_FWK_MODULE(GedGsfElectronRegTreeMaker);
